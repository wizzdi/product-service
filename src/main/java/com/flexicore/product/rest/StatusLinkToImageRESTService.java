...
public class StatusLinkToImageRESTService implements RestServicePlugin {
	...
	public PaginationResponse<StatusLinkToImageContainer> getAllStatusLinksToImage(
		...
		PaginationResponse<StatusLinkToImage> paginationResponse = service
				.getAllStatusLinksToImage(filtering, securityContext);
		return new PaginationResponse<>(paginationResponse.getList()
				.parallelStream().map(f -> new StatusLinkToImageContainer(f))
				.collect(Collectors.toList()), filtering,
				paginationResponse.getTotalRecords());
	...
	public StatusLinkToImage createStatusLinkToImage(
		...
		return service.createStatusLinkToImage(statusLinksToImageCreate,
				securityContext);
	}
	...
	public StatusLinkToImage updateStatusLinkToImage(
			...
			@Context SecurityContext securityContext) {
		StatusLinkToImage statusLinkToImage = statusLinksToImageUpdate.getId() != null
				? service.getByIdOrNull(statusLinksToImageUpdate.getId(),
						StatusLinkToImage.class, null, securityContext) : null;
		if (statusLinkToImage == null) {
			throw new BadRequestException(
					"No StatusLinksToImageUpdate with id "
							+ statusLinksToImageUpdate.getId());
		}
		...
		return service.updateStatusLinkToImage(statusLinksToImageUpdate,
				securityContext);
	}

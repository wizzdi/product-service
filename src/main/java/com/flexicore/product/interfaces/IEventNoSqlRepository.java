...
public interface IEventNoSqlRepository extends PluginRepository {
	...
	static Bson getEventsPredicate(EventFiltering eventFiltering) {
		...
		if (eventFiltering.getBaseclass() != null
				&& !eventFiltering.getBaseclass().isEmpty()) {
			Set<String> baseclasIds = eventFiltering.getBaseclass()
		...
		if (eventFiltering.getTargetBaseclass() != null
				&& !eventFiltering.getTargetBaseclass().isEmpty()) {
			Set<String> baseclasIds = eventFiltering.getTargetBaseclass()
					.parallelStream().map(f -> f.getId())
					.collect(Collectors.toSet());
			Bson in = in(TARGET_BASECLASS_ID, baseclasIds);
			pred = pred == null ? in : and(pred, in);
		}
		if (eventFiltering.getAckedUsers() != null
				&& !eventFiltering.getAckedUsers().isEmpty()) {
			Set<String> userIds = eventFiltering.getAckedUsers()
					.parallelStream().map(f -> f.getId())
					.collect(Collectors.toSet());
		...
		if (eventFiltering.getTenantIds() != null
				&& !eventFiltering.getTenantIds().isEmpty()) {
			Set<String> tenantsIds = eventFiltering.getTenantIds()
					.parallelStream().map(f -> f.getId())
					.collect(Collectors.toSet());
		...
		if (eventType != null) {
			Set<String> names = InheritanceUtils
					.listInheritingClassesWithFilter(
							new GetClassInfo().setClassName(eventType))
					.getList().parallelStream()
					.map(f -> f.getClazz().getCanonicalName())
					.collect(Collectors.toSet());
			names.add(eventType);
		...
		if (eventFiltering.getLocationArea() != null) {
			Bson eq = and(
					gte(BASECLASS_LAT, eventFiltering.getLocationArea()
							.getLatStart()),
					gte(BASECLASS_LON, eventFiltering.getLocationArea()
							.getLonStart()),
					lte(BASECLASS_LAT, eventFiltering.getLocationArea()
							.getLatEnd()),
					lte(BASECLASS_LON, eventFiltering.getLocationArea()
							.getLonEnd()));
			pred = pred == null ? eq : and(pred, eq);
		}
		...
		if (eventFiltering.getBaseclassNameLike() != null) {
			...
			Bson eq = Filters.eq(BASECLASS_NAME,
					eventFiltering.getBaseclassNameLike());
			pred = pred == null ? eq : and(pred, eq);
		}

		if (eventFiltering.getHumanReadableTextLike() != null) {

			Bson eq = Filters.eq(HUMAN_READABLE_TEXT,
					eventFiltering.getHumanReadableTextLike());
			pred = pred == null ? eq : and(pred, eq);
		}
	...
	<T extends Event> List<T> getAllEvents(EventFiltering eventFiltering,
			Class<T> c);

	<T extends Alert> List<T> getAllAlerts(AlertFiltering eventFiltering,
			Class<T> c);
}

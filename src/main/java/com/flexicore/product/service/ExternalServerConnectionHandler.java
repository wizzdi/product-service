...
public class ExternalServerConnectionHandler implements ServicePlugin {
    ...
    public <S extends ExternalServer, C extends ConnectionHolder<S>> void handleConfiguration(
        ...
        for (S externalServer : configurationCasted.getCache()) {
            if (ExternalServerConnectionManager.shouldInspectAll(externalServer)) {
                inspectAll(securityContext, connected, disconnected, configurationCasted, externalServer);
            }
            LocalDateTime now = LocalDateTime.now();
            List<SingleInspectJob<S, C>> toInspect = configurationCasted.getSingleInspectJobs().stream().filter(f -> externalServer.getId().equals(f.getExternalServerId())).filter(f -> f.getTimeToInspect().isBefore(now)).collect(Collectors.toList());
            try {
                for (SingleInspectJob<S, C> scSingleInspectJob : toInspect) {
                    inspectSingle(securityContext, configurationCasted, externalServer, scSingleInspectJob);
                }
            } finally {
    ...
    public <S extends ExternalServer, C extends ConnectionHolder<S>> void inspectAll(SecurityContext securityContext, ProductStatus connected, ProductStatus disconnected, ExternalServerConnectionConfiguration<S, C> configurationCasted, S externalServer) {
    ...
    public <S extends ExternalServer, C extends ConnectionHolder<S>> void inspectSingle(SecurityContext securityContext, ExternalServerConnectionConfiguration<S, C> configurationCasted, S externalServer, SingleInspectJob<S, C> singleInspectJob) {
        logger.info("preparing single inspect for " + externalServer.getName() + " (" + externalServer.getId() + ")"+ ", equipment id:" + singleInspectJob.getId());
        try {
            String id = externalServer.getId();
            C connectionHolder = configurationCasted.getConnectionHolders().get(id);
            if (connectionHolder == null) {
            ...
            logger.info("Starting Single Inspect for " + externalServer.getName() + " (" + externalServer.getId() + ")" + ", equipment id:" + singleInspectJob.getId());
            GenericInspectResponse genericInspectResponse = configurationCasted.getOnSingleInspect().apply(singleInspectJob.setConnectionHolder(connectionHolder));
            logger.info("Single Inspect ended with" + genericInspectResponse + " for " + externalServer.getName() + " (" + externalServer.getId() + ")" + ", equipment id:" + singleInspectJob.getId());


        } catch (Exception e) {
            logger.log(Level.SEVERE, "failed single inspecting server " + externalServer.getName() + " (" + externalServer.getId() + ")", e);
        }
    }
